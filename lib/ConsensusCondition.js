 ...
 */
// * @property {string} iroha.host Iroha host to connect to
// * @property {'grpc'} iroha.protocol protocol to use for Iroha
 ...
 * genesis block.
 *
 * @param {MeldIrohaConfig} config
 * @param {object} iroha required parameters for initialisation
 * @param {string} iroha.defaultRole e.g. `'user'`
 * @param {string} iroha.adminId e.g. `'admin@test'`
 * @param {string} iroha.adminEd25519PrivateKey
 * @param {string} iroha.firstPrincipalEd25519PublicKey first signatory user key
 * @param {object} iroha.commandService Iroha command service
 */
function initIrohaMeldDomain(config, iroha) {
  return new BatchBuilder([
    new TxBuilder()
      .createDomain({
        domainId: config['@domain'],
        defaultRole: iroha.defaultRole
      })
      .addMeta(iroha.adminId, 1)
      .tx,
    new TxBuilder()
      ...
      .createAccount({
        ...
        accountName: 'clone',
        domainId: config['@domain'],
        publicKey: iroha.firstPrincipalEd25519PublicKey
      })
      .addMeta(iroha.adminId, 1)
      .tx
  ]).setBatchMeta(0)
    .sign([iroha.adminEd25519PrivateKey], 0)
    .sign([iroha.adminEd25519PrivateKey], 1)
    .send(iroha.commandService);
}
...
class ConsensusCondition {
  ...
  static declare = (id) => ({
    ...
    'http://js.m-ld.org/#require': '@m-ld/m-ld-iroha',
